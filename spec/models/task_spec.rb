...
RSpec.describe Task, type: :model do
  ...
  it "【Scope】titleに部分一致の検索が行えているか" do
    create(:task, id: 1, title: "hoge")
    create(:task, id: 2, title: "test")
    create(:task, id: 3, title: "huga")
  ...
  it "【Scope】statusの完全一致の検索が行えているか" do
    create(:task, id: 1, status: 0)
    create(:task, id: 2, status: 1)
    create(:task, id: 3, status: 2)
    create(:task, id: 4, status: 3)
    task1 = Task.all.status_equal("2").first
    expect(task1.id).to eq 3
  end
  it "【Scope】priorityの完全一致の検索が行えているか" do
    create(:task, id: 1, priority: 0)
    create(:task, id: 2, priority: 1)
    create(:task, id: 3, priority: 2)
    task1 = Task.all.priority_equal("2").first
    expect(task1.id).to eq 3
  end
  it "【Scope】created_atのOrderByが効いているか" do
    create(:task, id: 1, created_at: Time.current + 1.days)
    create(:task, id: 2, created_at: Time.current + 4.days)
    create(:task, id: 3, created_at: Time.current + 2.days)
    create(:task, id: 4, created_at: Time.current + 3.days)
    expect(Task.all.created_before.map(&:id)).to eq [2,4,3,1]
  end
  
  it "【Method】limitのOrderByが効いているか" do
    create(:task, id: 1, limit: Time.current + 1.days)
    create(:task, id: 2, limit: Time.current + 4.days)
    create(:task, id: 3, limit: Time.current + 2.days)
    create(:task, id: 4, limit: Time.current + 3.days)
